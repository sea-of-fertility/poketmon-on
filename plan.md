...
- 화면 가장자리에서 방향 반사
- 60초 Idle 시 Sleep 전환, Sleep 상태에서 수면 애니메이션 재생
...
### Step 4-1. 마우스 이벤트 처리
- Phase 2의 hitTest로 포켓몬 영역에서만 이벤트 수신
...
- 드래그: Dragged 상태 전환, 마우스 좌표 따라 포켓몬 위치 이동. 놓으면 → Idle
...
- 몬스터볼 아이콘 (16×16 Template 이미지) 적용
...
### Step 6-3. 세대 탭
- 5개 세그먼트: Gen 1 (151종) / Gen 2 (100종) / Gen 3 (135종) / Gen 4 (107종) / Gen 5 (140종)
...
### Step 6-7. 선택 완료 / 취소
- 선택 완료: SpriteAnimator에 새 포켓몬 ID 전달 → 데스크톱 펫 교체. 윈도우 닫기
- 취소: 변경 없이 윈도우 닫기
...
- 투명도: 슬라이더 30% ~ 100% (기본 100%). 스프라이트 불투명도
- 표시 위치: 드롭다운 — 항상 위 / 일반 / 바탕화면만 (ui-mockup.html에 추가된 항목)
...
- ObservableObject로 구현하여 SwiftUI 바인딩 + Phase 3 이동 로직에 반영
